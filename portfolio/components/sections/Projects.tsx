...
// Couleurs officielles des technos
...
const statusColor: Record<Project["status"], string> = {
  ...
  archived: "text-[#444] border-[#2a2a2a] bg-transparent",
};
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-1.5">
          <span className="font-[Cormorant_Variable] text-[#2a2a2a] text-4xl font-light leading-none select-none">
        ...
        <span
          className={`shrink-0 mt-1 px-3 py-1 text-[10px] tracking-[0.18em] uppercase border rounded-full ${statusColor[project.status]}`}
        >
      ...
      {/* Description */}
      <p className="text-[#555] text-[13px] leading-[1.85] tracking-[0.02em] flex-1">
        {project.description}
      </p>
      ...
      <div className="flex flex-wrap gap-2">
        {project.tags.map((tag) => {
          ...
          return (
            <span
              key={tag}
              className="flex items-center gap-1.5 text-[10px] tracking-[0.15em] uppercase text-[#444] border border-[#1e1e1e] px-2.5 py-1"
            >
              {tech && (
      ...
      <div className="flex items-center justify-between pt-2 border-t border-[#141414]">
        <span className="text-[11px] tracking-[0.2em] text-[#333] uppercase">
          {project.year}
        </span>
        ...
        <div className="flex items-center gap-4">
          {project.github && (
            <Link
              ...
              className="text-[11px] tracking-[0.18em] uppercase text-[#444] hover:text-[#c9a84c] transition-colors duration-300"
            >
              GitHub
            </Link>
          )}
